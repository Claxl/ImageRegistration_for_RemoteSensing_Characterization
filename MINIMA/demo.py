    ...
    if ret_H is not None:
        ...
        #    fig_path = osp.join(figures_dir, f"{img0_name}_after_homography_{method}.jpg")
         #   cv2.imwrite(fig_path, im0)
            ...
            dict = {'NM': len(match_res),
                    'NCM': len(inliers),
                    'ratio': len(inliers) / len(match_res) if len(match_res) != 0 else 0,
                    'reg_time': end,
                    'transformation_matrix': ret_H.squeeze().cpu().numpy().tolist(),
                    'inliers': inliers.tolist(),
                    'registered_img': None,
                    'mosaic_img': None,
                    'matches_img': None,
                    'keypoints_sar': mkpts0,
                    'keypoints_opt': mkpts1,
                    'good_matches': inliers,
                    }
    return dict if dict is not None else None
