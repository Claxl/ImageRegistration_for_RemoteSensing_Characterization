...
def load_roma(args, test_orginal_megadepth=False):
    ...
    from MINIMA.src.third_party.RoMa.romatch import roma_outdoor
    from MINIMA.src.third_party.RoMa.romatch import tiny_roma_v1_outdoor
    if test_orginal_megadepth:
        from MINIMA.src.config.default_for_megadepth_dense import get_cfg_defaults
    else:
        from MINIMA.src.config.default import get_cfg_defaults
...
def load_loftr(args, test_orginal_megadepth=False):
    from MINIMA.src.third_party.LoFTR.src.loftr import LoFTR, default_cfg
    if test_orginal_megadepth:
        from MINIMA.src.config.default_for_megadepth_dense import get_cfg_defaults
    else:
        from MINIMA.src.config.default import get_cfg_defaults
    from MINIMA.src.utils.data_io_loftr import DataIOWrapper, lower_config
    config = get_cfg_defaults(inference=True)
    config = lower_config(config)
...
def load_sp_lg(args, test_orginal_megadepth=False):
    from MINIMA.src.third_party.LightGlue.lightglue import LightGlue, SuperPoint
    from MINIMA.src.third_party.LightGlue.lightglue.utils import rbd
    if test_orginal_megadepth:
        from MINIMA.src.config.default_for_megadepth_sparse import get_cfg_defaults
    else:
        from MINIMA.src.config.default import get_cfg_defaults
    from MINIMA.src.utils.data_io_sp_lg import DataIOWrapper, lower_config
